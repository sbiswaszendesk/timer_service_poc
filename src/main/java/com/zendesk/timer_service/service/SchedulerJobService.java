package com.zendesk.timer_service.service;
...
import com.zendesk.timer_service.component.JobScheduleCreator;
import com.zendesk.timer_service.entity.SchedulerJobInfo;
import com.zendesk.timer_service.job.SampleCronJob;
import com.zendesk.timer_service.job.SimpleJob;
import com.zendesk.timer_service.repository.SchedulerRepository;
...
public class SchedulerJobService {
	...
	public String saveOrupdate(SchedulerJobInfo scheduleJob) throws Exception {
		String jobId = "";
	  if (scheduleJob.getCronExpression().length() > 0) {
			...
			scheduleJob.setRepeatTime((long) scheduleJob.getRepeatTime());
		}
	...
	private void scheduleNewJob(SchedulerJobInfo jobInfo) {
		try {
			...
			if (!scheduler.checkExists(jobDetail.getKey())) {
				...
				jobDetail.getJobDataMap().put("id", jobInfo.getJobName());
        jobDetail.getJobDataMap().put("payload", jobInfo.getPayLoad());

				Trigger trigger;
				System.out.println("skb " + jobInfo.getCronExpression());
				if (jobInfo.getCronJob()) {
					trigger = scheduleCreator.createCronTrigger(jobInfo.getJobName(), new Date(),
							jobInfo.getCronExpression(), SimpleTrigger.MISFIRE_INSTRUCTION_FIRE_NOW);
				} else {
					trigger = scheduleCreator.createSimpleTrigger(jobInfo.getJobName(), new Date(jobInfo.getStartTime()),
